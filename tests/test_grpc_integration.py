...
import fluxgraph_pb2 as pb  # noqa: E402
import fluxgraph_pb2_grpc as pb_grpc  # noqa: E402
...
def test_load_config(stub, config_path):
    ...
    request = pb.ConfigRequest(
        config_content=config_content, format="yaml", config_hash="test_config_v1"
    )
...
def test_load_config_with_hash(stub, config_path, config_hash):
    ...
    request = pb.ConfigRequest(
        config_content=config_content, format="yaml", config_hash=config_hash
    )
...
def test_register_provider(stub, provider_id, device_ids):
    ...
    request = pb.ProviderRegistration(provider_id=provider_id, device_ids=device_ids)
...
def test_read_signals(stub):
    ...
    request = pb.SignalRequest(paths=["chamber.temp", "sensor.reading", "display.temp"])
