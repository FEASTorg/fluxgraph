...
TEST(MultiModelTest, TwoThermalMassesWithHeatTransfer) {
    ...
    // Chamber B should remain at initial (no heat transfer in this simplified model)
    // Note: This test demonstrates multi-model execution, not full thermal coupling
...
TEST(MultiModelTest, EdgesBetweenModels) {
    ...
    EXPECT_NEAR(observed_temp, source_temp, 2.0);  // Filtered value should track source
}
