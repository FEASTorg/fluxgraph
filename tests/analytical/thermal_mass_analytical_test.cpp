...
TEST(ThermalMassAnalytical, ExponentialDecay) {
  ...
  ThermalMassModel model("test", C, h, T0, "model/temperature",
...
TEST(ThermalMassAnalytical, HeatingToEquilibrium) {
  ...
  double T_eq =
      T_amb +
      P / h; // Equilibrium: 20 + 50/10 = 25 degC... wait, T0=25, so no change
