...
ITransform* GraphCompiler::parse_transform(const TransformSpec& spec) {
    ...
    if (type == "linear") {
        ...
        uint32_t seed = static_cast<uint32_t>(std::get<int64_t>(spec.params.at("seed")));
        ...
        size_t window_size = static_cast<size_t>(std::get<int64_t>(spec.params.at("window_size")));
...
IModel* GraphCompiler::parse_model(const ModelSpec& spec, SignalNamespace& ns) {
    ...
    if (type == "thermal_mass") {
        ...
        double heat_transfer_coeff = std::get<double>(spec.params.at("heat_transfer_coeff"));
        double initial_temp = std::get<double>(spec.params.at("initial_temp"));
        std::string temp_path = std::get<std::string>(spec.params.at("temp_signal"));
        std::string power_path = std::get<std::string>(spec.params.at("power_signal"));
        std::string ambient_path = std::get<std::string>(spec.params.at("ambient_signal"));

        return new ThermalMassModel(spec.id, thermal_mass, heat_transfer_coeff,
                                   initial_temp, temp_path, power_path, ambient_path, ns);
    }
    else {
...
void GraphCompiler::topological_sort(std::vector<CompiledEdge>& edges) {
    ...
    while (!ready.empty()) {
        ...
        for (size_t idx : outgoing[sig]) {
            if (processed_edges.count(idx)) continue;
    ...
    if (sorted.size() != edges.size()) {
        throw std::runtime_error("GraphCompiler: Topological sort failed (possible cycle)");
    }
...
void GraphCompiler::detect_cycles(const std::vector<CompiledEdge>& edges) {
    ...
    for (const auto& [node, _] : graph) {
        if (!visited.count(node)) {
            if (has_cycle(node)) {
                throw std::runtime_error("GraphCompiler: Cycle detected (algebraic loop). Add DelayTransform to feedback path.");
            }
        }
    }
}
...
void GraphCompiler::validate_stability(const std::vector<std::unique_ptr<IModel>>& models, double expected_dt) {
    for (const auto& model : models) {
        ...
        if (expected_dt > limit) {
            ...
            oss << "Stability violation: " << model->describe()
                << " requires dt < " << limit << "s, but dt = " << expected_dt << "s";
